...
PinedBlog()
    ...
    const PinedBlog = async ()=>{
        setLoader(true)
      await  http.get('shihab/pinedblog')
      .then(res=>{
...
<b className="text-5xl font-semibold  text-sky-400"> <span className="text-orange-400">PINED</span> -Blogs</b>

<div className="grid grid-cols-1 gap-3 md:grid-cols-2 mt-9 lg:grid-cols-3">
    ...
    {!loader?
        Object.values(pined).map((blog)=>{
          return(
            ...
            />
            <Link to={`/Blogs/viewblog/` + blog._id} className='text-left py-1'><span className='text-gray-200 my-1 text-left w-full cursor-pointer hover:text-sky-300'>{blog.title ? blog.title: "No paragraph found"}</span></Link>
            ...
            <div className="flex items-center gap-1">  <BiLike className='cursor-pointer animate-bounce text-3xl text-fuchsia-500'/> <span className="text-green-400 text-xs">232</span></div>
              ...
              <div className="flex items-center gap-1">      <BiShare  className='cursor-pointer text-cyan-400'/> <span className="text-green-400 text-xs">232</span></div>
